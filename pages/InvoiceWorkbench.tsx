...
export const InvoiceWorkbench: React.FC<InvoiceWorkbenchProps> = ({
  ...
  const getStatusBadge = (status: InvoiceStatus) => {
    switch (status) {
      case InvoiceStatus.APPROVED:
        return (
          <span className="px-2 py-1 rounded-sm text-xs font-bold bg-teal-100 text-teal-700 uppercase tracking-wide border border-teal-200">
            APPROVED
          </span>
        );
      case InvoiceStatus.EXCEPTION:
        return (
          <span className="px-2 py-1 rounded-sm text-xs font-bold bg-red-100 text-red-700 uppercase tracking-wide border border-red-200">
            EXCEPTION
          </span>
        );
      case InvoiceStatus.PENDING:
        return (
          <span className="px-2 py-1 rounded-sm text-xs font-bold bg-orange-100 text-orange-700 uppercase tracking-wide border border-orange-200">
            PENDING
          </span>
        );
      default:
        return (
          <span className="px-2 py-1 rounded-sm text-xs font-bold bg-gray-100 text-gray-700 uppercase tracking-wide border border-gray-200">
            {status}
          </span>
        );
    }
  ...
  return (
    <div
      className="h-full flex flex-col p-8 font-sans overflow-hidden bg-[#F3F4F6] relative"
        ...
        <div className="flex items-center space-x-4 bg-white px-4 py-2 rounded-sm border border-gray-200 shadow-sm">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-lg font-bold text-teal-700 font-mono">
              $
              {totalListValue.toLocaleString("en-US", {
